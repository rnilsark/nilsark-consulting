...
Create a blank state.md from the template below and upload it to the `.nilsark` subfolder of the Drive month folder before proceeding.
...
Free-form key-value section updated by each command. `Month-close sent: yes` is the flag that prevents accidental re-running of `/month-close`.
