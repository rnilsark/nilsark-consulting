...
2. **Read both calendars** (one `events list` per calendar):
   ```bash
   gws calendar events list --params '{"calendarId":"richard@nilsark.com","timeMin":"<RFC3339>","timeMax":"<RFC3339>","singleEvents":true,"orderBy":"startTime"}'
   gws calendar events list --params '{"calendarId":"richard.nilsark@gmail.com","timeMin":"<RFC3339>","timeMax":"<RFC3339>","singleEvents":true,"orderBy":"startTime"}'
   ```

3. **Conflict check across BOTH calendars**:
   ```bash
   gws calendar freebusy query --json '{"timeMin":"<RFC3339>","timeMax":"<RFC3339>","items":[{"id":"richard@nilsark.com"},{"id":"richard.nilsark@gmail.com"}]}'
   ```
